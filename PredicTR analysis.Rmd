...
loglik_reduced <- logLik(new_fit_val)
loglik_full <- logLik(s_fit_val)
...
test_val_data$risk_surgery_new <- factor(
  paste0(
...
surv_fit <- survfit(surv_obj ~ risk_surgery_new, data = test_val_data)
...
```
```{r Surgery Plot: Validation set - Three-Biomarker Model (not adjusted)}
# Main plot with no risk table
#Kaplan-Meier
surv_obj2 <- Surv(test_val_data$t.os, test_val_data$Outcome)

surv_fit2 <- survfit(surv_obj2 ~ risk_surgery_new, data = test_val_data)

summary(surv_fit2, times = seq(0, 5) * 1)
...
ggsurv <- ggsurvplot(
  ...
  palette = c("blue", "green", "red", "purple"),
  risk.table = FALSE,
  title = "Validation set (Surgery) - Three-Biomarker Model",
  xlab = "Time (years)",
  ylab = "Probability (overall survival)",
  legend.title = " ",
  legend.labs = c("Low, Surgery-", "Low, Surgery+", "High, Surgery-", "High, Surgery+"),
  conf.int = FALSE,
  pval = FALSE,
...
####################################
#PLOT
test_val_data$risk_surgery_new <- factor(test_val_data$risk_surgery_new,
  levels = c("ls0", "ls1", "hs0", "hs1"),
  labels = c("L-S0", "L-S1", "H-S0", "H-S1")
)

surv_fit2 <- survfit(surv_obj ~ risk_surgery_new, data = test_val_data)
names(surv_fit2$strata) <- c("L-S0", "L-S1", "H-S0", "H-S1")
...
p1 <- plot_main$plot
p1 <- p1 +
  annotate("text", x = 0.1, y = 0.2, hjust = 0, size = 5,
           label = "Low, Surgery+ vs Low, Surgery-:") +
  annotate("text", x = 0.1, y = 0.15, hjust = 0, size = 5,
           label = "HR = 0.78 (0.35 - 1.72), P = 0.54") +
  annotate("text", x = 0.1, y = 0.1, hjust = 0, size = 5,
           label = "High, Surgery+ vs High, Surgery-:") +
  annotate("text", x = 0.1, y = 0.05, hjust = 0, size = 5,
           label = "HR = 0.42 (0.25-0.70), P < 0.001")
p2 <- plot_table$table
plot_new_val_surg <- p1 / p2 + plot_layout(heights = c(5, 1))
plot_new_val_surg

ggsave("survival_plot_val.png", plot = plot_new_val_surg, height = 8, width = 10, dpi = 300)
...
complete_training_data$risk_surgery_new
#Kaplan-Meier
...
ggsurv <- ggsurvplot(
  ...
  palette = c("blue", "green", "red", "purple"),
  risk.table = TRUE,
  title = "Training set (Surgery) - Three-Biomarker Model",
  xlab = "Time (years)",
  ylab = "Survival Probability",
  legend.title = "Group",
  legend.labs = c("Low, Surgery-", "Low, Surgery+", "High, Surgery-", "High, Surgery+"),
  ...
  risk.table.height = 0.5,  # Try increasing this if space is tight
  font.tickslab = 10  
)
...
```{r Surgery Plot: Training set - Three-Biomarker Model (not adjusted)}
...
surv_fit1 <- survfit(surv_obj0 ~ risk_surgery_new, data = complete_training_data)
# Main plot with no risk table

complete_training_data$risk_surgery_new <- factor(complete_training_data$risk_surgery_new,
  levels = c("ls0", "ls1", "hs0", "hs1"),
  labels = c("L-S0", "L-S1", "H-S0", "H-S1")
)

summary(complete_training_data$risk_surgery_new)

names(surv_fit1$strata) <- c("L-S0", "L-S1", "H-S0", "H-S1")
...
p1 <- p1 +
  annotate("text", x = 0.1, y = 0.2, hjust = 0, size = 5,
           label = "Low, Surgery+ vs Low, Surgery-:") +
  annotate("text", x = 0.1, y = 0.15, hjust = 0, size = 5,
           label = "HR = 0.17 (0.05 - 0.59), P = 0.002") +
  annotate("text", x = 0.1, y = 0.1, hjust = 0, size = 5,
           label = "High, Surgery+ vs High, Surgery-:") +
  annotate("text", x = 0.1, y = 0.05, hjust = 0, size = 5,
           label = "HR = 0.55 (0.30 - 1.00), P = 0.05")
p2 <- plot_table$table
plot_new_train_surg <- p1 / p2 + plot_layout(heights = c(5, 1))
plot_new_train_surg
ggsave("survival_plot.png", plot = plot_new_train_surg, height = 8, width = 10, dpi = 300)
```
