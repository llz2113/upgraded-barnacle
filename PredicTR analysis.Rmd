...
complete_training_data$risk_score <- predict(survivin_model, complete_training_data, type = "risk")
...
complete_val_data_dx_date$risk_score <- predict(survivin_model, complete_val_data_dx_date, type = "risk")
...
test_val_data$risk_score <- predict(survivin_model, test_val_data, type = "risk")
...
val_data_refined <- complete_val_data_dx_date %>%
  filter(if_all(all_of(biomarkers_reduced, t.os), ~ !is.na(.)))
...
test_val_data$risk_score_new <- predict(new_model, test_val_data, type = "risk")
...
cox_model <- coxph(Surv(t.os, Outcome) ~ risk_group, data = test_val_data)
...
complete_training_data$risk_score <- predict(survivin_model, newdata = complete_training_data, type = "risk")
...
#Risk scores
test_val_data$risk_score_new <- predict(new_model, newdata = test_val_data, type = "risk")
...
cox_model_new <- coxph(Surv(t.os, Outcome) ~ risk_group_new, data = test_val_data)
